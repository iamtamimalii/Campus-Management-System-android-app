...
class facultyDisplay : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        view.setOnClickListener {
            ...
            db.child("Faculty").addValueEventListener(object: ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (obj in snapshot.children){
                        val temp = obj.getValue<faculty>()
                        val facultyData ="First Name: ${temp?.fName}\nLast Name: ${temp!!.lName}\nGender: ${temp.gender}\nReg. Number: ${temp.reg}\nUniversity: ${temp.uni}\n" +
                                "Faculty: ${temp.fac}"
                        dataList.add(facultyData)
                    }
                    myListAdapter=ArrayAdapter<String>(this@facultyDisplay, R.layout.customlistview, R.id.text_view_title1, dataList)
                    list.adapter=myListAdapter
                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@facultyDisplay, "Failed to Load Data", Toast.LENGTH_SHORT).show()
                }
            })
            progress.cancel()
        }
